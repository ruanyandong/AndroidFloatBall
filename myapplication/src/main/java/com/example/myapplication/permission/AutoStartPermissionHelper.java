...
import static com.qukandian.sdk.config.BaseSPKey.KEY_IS_AUTO_START_HAD;

public class AutoStartPermissionHelper {

    private static boolean isHadAutoStart = false;

    public static void setIsHadAutoStart(boolean isAutoStart) {
        if (isHadAutoStart != isAutoStart) {
            SpUtil.putData(KEY_IS_AUTO_START_HAD, isAutoStart);
        }
    ...
    public static boolean checkAutoStartPermission() {
        try {
            isHadAutoStart = SpUtil.getData(KEY_IS_AUTO_START_HAD, false);
